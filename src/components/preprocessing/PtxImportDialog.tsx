// Pick a single .ptx / .pts file and register it with PointCloudLabeler.
//
// The Rust side walks every scan section's header (ncols, nrows,
// scanner position + axes, 4×4 transform), records the byte offset of
// each section's point block so the export step can seek directly to
// it, and writes a manifest under
// <projectFolder>/preprocessing/ptx/<safeName>/manifest.json.
...
export default function PtxImportDialog({ onClose, onImported, onStatus }: Props) {
  ...
  const browse = async () => {
    ...
    const res = await api.openFileDialog({
      filters: [
        { name: 'PTX / PTS point cloud', extensions: ['ptx', 'pts', 'PTX', 'PTS'] },
      ],
    });
    if (!res?.path) return;
    ...
    if (!name.trim()) {
      ...
      setName(base.replace(/\.(ptx|pts)$/i, ''));
    }
  };
  ...
  const run = async () => {
    ...
    try {
      const api = (window as unknown as Record<string, unknown>).desktop as {
        ptxImportProject?: (args: { projectFolder: string; sourcePath: string; name: string }) => Promise<{ id: string }>;
      } | undefined;
      if (!api?.ptxImportProject) throw new Error('PTX import needs the desktop build (ptx_import_project command missing).');
      const res = await api.ptxImportProject({
        projectFolder: project.folder,
        sourcePath: sourcePath.trim(),
        name: name.trim() || 'PTX project',
      });
      onStatus({ kind: 'ok', msg: `Imported PTX/PTS file "${name.trim() || res.id}".` });
      await onImported();
    } catch (e) {
  ...
  return (
    <div
      ...
      role="dialog"
      aria-label="Import PTX / PTS file"
    >
      <div className="panel rounded-xl overflow-hidden" style={{ width: 560, maxWidth: '94vw' }}>
        <div className="flex items-center gap-3 px-5 py-4" style={{ borderBottom: '1px solid var(--line)', background: 'var(--wash-1)' }}>
          ...
          <div className="flex-1 min-w-0">
            <div className="text-[14px] font-semibold" style={{ color: 'var(--text)' }}>Import PTX / PTS file</div>
            <div className="mono text-[10.5px] truncate" style={{ color: 'var(--text-mute)' }}>
              Leica Cyclone ASCII — also Topcon MAGNET Collage, 3D Forest, NavVis exports
            </div>
          </div>
        ...
        <div className="px-5 py-4 flex flex-col gap-4">
          <Field label="File">
            ...
            <div className="mono text-[10px] mt-1.5" style={{ color: 'var(--text-mute)', lineHeight: 1.5 }}>
              A single .ptx or .pts file. PTX carries a per-scan 4×4 transform; PTS is a single point dump.
            </div>
          </Field>
