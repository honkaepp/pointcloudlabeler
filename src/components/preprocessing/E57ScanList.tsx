// E57 scan-position list — one row per Data3D section in the imported
// .e57 file. Pick which scans to export, optionally crop to a world bbox
// (post-pose / project frame, same convention as the Riegl panel) and
// merge into a single LAS / LAZ the Editor module can open.

import { useMemo, useState } from 'react';
import { useProject } from '../../context/ProjectContext';
import type { E57ProjectSummary, E57ScanPosition } from './E57ProjectPanel';
import { confirmDialog } from '../../ui/dialogs';

interface Props {
  project: E57ProjectSummary;
  onChange: () => void | Promise<void>;
  onStatus: (s: { kind: 'ok' | 'err' | 'info'; msg: string } | null) => void;
}

export default function E57ScanList({ project, onChange, onStatus }: Props) {
  ...
  // Listen for the e57-progress event the Rust side emits during export.
  // Wired up only while the dialog is busy so we don't leak listeners.
  // Uses the global tauri event API if present.
  ...
  const doExport = async () => {
    ...
    if (!api?.e57ExportRegion || !api?.saveLasDialog) {
      onStatus({ kind: 'err', msg: 'E57 region export needs the desktop build.' });
      return;
    }
    ...
    onStatus({ kind: 'info', msg: 'Exporting E57 scan positions…' });
    try {
      const parseN = (s: string): number | null => {
        const v = parseFloat(s);
        return Number.isFinite(v) ? v : null;
      };
      const res = await api.e57ExportRegion({
      ...
      onStatus({ kind: 'err', msg: `E57 export failed: ${e instanceof Error ? e.message : String(e)}` });
    } finally {
  ...
  const remove = async () => {
    if (!pointcloudlabeler?.folder) return;
    if (!await confirmDialog(`Forget E57 file "${project.name}"? The original .e57 stays on disk; only PointCloudLabeler's manifest is removed.`)) return;
    const api = (window as unknown as Record<string, unknown>).desktop as {
      e57RemoveProject?: (args: { projectFolder: string; e57Id: string }) => Promise<void>;
    } | undefined;
    if (!api?.e57RemoveProject) {
      onStatus({ kind: 'err', msg: 'Cannot remove — desktop command missing.' });
      return;
    }
    try {
      await api.e57RemoveProject({ projectFolder: pointcloudlabeler.folder, e57Id: project.id });
      onStatus({ kind: 'ok', msg: `Removed E57 file "${project.name}".` });
      await onChange();
    } catch (e) {
    ...
    <div className="flex flex-col">
        ...
        <div className="flex items-start gap-3">
          <div className="flex-1 min-w-0">
            ...
            <div className="mono text-[10px] mt-1" style={{ color: 'var(--text-mute)' }}>
              ...
              {project.libraryVersion ? ` · ${project.libraryVersion}` : ''}
            </div>
            {project.coordinateMetadata && (
              <div className="mono text-[9.5px] mt-0.5 truncate" title={project.coordinateMetadata} style={{ color: 'var(--text-mute)' }}>
                CRS: {project.coordinateMetadata}
              </div>
            )}
          </div>
          <button className="btn !h-7 !px-2 mono text-[10.5px]" onClick={remove} title="Forget this E57 file (leaves the original alone)">Remove</button>
        </div>
      </div>
          ...
          <div className="max-h-[260px] overflow-y-auto scroll-thin">
            {project.scanPositions.length === 0 ? (
              <div className="p-4 mono text-[10.5px] text-center" style={{ color: 'var(--text-mute)' }}>
                No scan positions in this file. (The .e57 carries no Data3D sections.)
              </div>
            ) : (
      ...
      <div className="px-4 py-3" style={{ borderTop: '1px solid var(--line)' }}>
        ...
        <div className="mono text-[9.5px] mt-1" style={{ color: 'var(--text-mute)', lineHeight: 1.5 }}>
          Leave any field blank to skip that axis. The crop is applied AFTER each scan's pose, so the coords live in the file's shared world frame.
        </div>
      </div>
      ...
      <div className="px-4 py-3" style={{ borderTop: '1px solid var(--line)' }}>
        <button
          ...
          title={anySelected ? 'Read points from the selected scans, apply each scan\'s pose, optionally crop and merge into a single LAS / LAZ' : 'Select at least one scan position'}
        >
          {exportBusy
...
function ScanRow({ sp, on, alt, onToggle }: {
  sp: E57ScanPosition; on: boolean; alt: boolean; onToggle: () => void;
}) {
  // E57 files declare per-scan attributes (intensity, color, cartesian /
  // spherical coords). Surface them as little tags so the user can spot
  // a scan that's intensity-only (no RGB) before exporting.
  const tags: string[] = [];
  if (sp.hasIntensity) tags.push('I');
  if (sp.hasColor) tags.push('RGB');
  if (sp.hasSpherical && !sp.hasCartesian) tags.push('sph');
...
function safeFilename(s: string): string {
  return s.replace(/[\\/:*?"<>|]+/g, '_').slice(0, 80) || 'e57';
}
